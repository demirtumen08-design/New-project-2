...
def fetch_latest_news(limit: int = 18) -> dict[str, Any]:
    ...
    for source in sources:
        try:
            ...
            status.append({"source": source.get("name", "Kaynak"), "status": "error", "message": str(exc)})

    deduped = dedupe_items(items)
    deduped.sort(key=lambda item: item["published_sort"], reverse=True)
    selected = deduped[: max(1, min(int(limit), 50))]
...
def make_item(
    ...
    if is_rejected_title(title):
        return None
    category = map_category(raw_category or str(source.get("category", "")) or "gundem")
    item = {
        "source": source["name"],
